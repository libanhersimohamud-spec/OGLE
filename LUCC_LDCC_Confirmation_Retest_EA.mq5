//+------------------------------------------------------------------+
//|              LUCC_LDCC_Confirmation_Retest_EA_V4.mq5             |
//|                                                                    |
...
input group "Breakeven Stop (V4 — optimizable)"
input bool   InpUseBreakeven         = true;  // Move the SL to breakeven once the trade reaches the R-trigger below
input double InpBreakevenTriggerR    = 1.5;   // Arm breakeven when floating profit reaches this many R (multiples of
                                               // the trade's OWN risk distance = |entry - initial SL|, NOT a fixed pip
                                               // value). This is the input to sweep in the Optimizer:
                                               // e.g. 1.00 / 1.25 / 1.50 / 1.75 / 2.00 (start 1.0, step 0.25, stop 2.5).
...
input group "Daily Confirmation Filter (confirms direction only)"
input bool   InpUseDailyConfirmation = true;  // Require >=1 Daily pattern (of the same seven) agreeing with
                                               // the trade direction before entry: a BUY needs a Daily BUY
                                               // pattern, a SELL needs a Daily SELL pattern. Daily does NOT
                                               // set bias or generate entries — confirmation only.
...
struct SPendingLog
{
   ...
   // --- V4 breakeven management ---
   string   outcome;          // "TP" / "BE" / "LOSS" — decided by exit PRICE, the source of truth for Result
...
void UpdateBreakeven(double bid, double ask)
{
   ...
   for(int i = 0; i < ArraySize(g_openTrades); i++)
   {
      if(g_openTrades[i].beArmed || g_openTrades[i].initRiskDist <= 0.0)
         continue;
...
void UpdateDailyBreakeven(double bid, double ask)
{
   ...
   for(int i = 0; i < ArraySize(g_openTrades); i++)
   {
      if(g_openTrades[i].beArmed)
         continue; // already at breakeven (R-based rule or a prior Daily break)
...
//| sign): TP = reached take-profit (the only true WIN); BE = stopped  |
//| at the armed breakeven level (neither win nor loss); LOSS = hit the |
//| original stop. Tolerance scales with the trade's risk so slippage  |
//| never mis-buckets a fill.                                          |
//+------------------------------------------------------------------+
...
bool TryOpen(bool isSell, SSignalState &st)
{
   ...
   if(ok)
   {
      ...
      // V4: freeze the trade's own 1R (entry->initial SL) and reset breakeven state.
      ot.initRiskDist = MathAbs(ot.entryPrice - sl);
      ot.beArmed      = false;
      ot.beLevelPrice = ot.entryPrice;   // provisional; finalised the moment BE actually arms
                      ...
                      bool firstWinOfBias, string outcome)
{
   ...
   g_pending[n].outcome        = outcome;          // V4: TP / BE / LOSS (source of truth for Result)
...
void UpdateWeeklyStats(datetime entryTime, double rRealized, string outcome)
{
   ...
   // V4: bucket by the price-based outcome (TP/BE/LOSS), so a BE exit with a
   // small +swap is counted as breakeven, never as a win. A BE also does NOT
   // break a win/loss streak's counting semantics inconsistently — it resets
   // both, exactly like the old flat-net breakeven case.
   if(outcome == "TP")
   {
...
void FinalizePendingLog(const SPendingLog &p, bool windowComplete)
{
   ...
   string   result        = (p.outcome == "TP") ? "WIN" : (p.outcome == "BE") ? "BE" : "LOSS";
                 ...
                 double grossProfit, double commission, double swap)
{
   ...
   //   TP   -> a real win: books the episode win + the daily/weekly win-stops.
   //   BE   -> breakeven: NEITHER win nor loss. Books nothing, so the EA keeps
   //           taking new valid setups under the SAME weekly bias.
   //   LOSS -> hit the original stop. (Losses never stopped the direction.)
   string outcome = ClassifyOutcome(t, exitPrice);
   bool   isTpWin = (outcome == "TP");

   // First win of THIS trade's weekly-bias episode? Tracked on a genuine TP only
   // (even when the toggle is off) so the "first winning trade" is identifiable.
   // The episode is the trade's own weekly-bias candle, so each week is an
   // independent instance and the restriction resets per weekly bias.
   bool firstWin = false;
   if(isTpWin)
   {
      ...
      if(t.isSell) g_sell.doneToday = true; else g_buy.doneToday = true; // daily win-stop — TP only
   }

   Dbg(StringFormat("%s CLOSE posId=%d net=%.2f outcome=%s%s%s episode=%s (openTrades left=%d)",
                    t.isSell ? "SELL" : "BUY", closedPosId, netProfit, outcome,
                    t.beArmed ? " [BE-armed]" : "", firstWin ? " [FIRST-WIN]" : "",
...
void UpdatePanel()
{
   ...
   PanelSet(10, StringFormat("TP 1:%.1f | SL x%.2f | BE %s @ %.2fR",
            InpTakeProfitRMultiple, InpStopLossMultiplier,
            InpUseBreakeven ? "ON" : "OFF", InpBreakevenTriggerR), clrGainsboro);
...
void OnTick()
{
   ...
   UpdateBreakeven(bid, ask);        // V4: move SL to breakeven once the R-trigger is reached
