//+------------------------------------------------------------------+
//| LUCC_LDCC_Confirmation_Retest_EA_3TIER_FILTER.mq5                 |
//|                                                                    |
//| Expert Advisor port of "LUCC / LDCC Confirmation & Retest         |
//| Indicator" (Pine v6).                                              |
//|                                                                    |
//| ==== 3-TIER BIAS FILTER (this build) ============================ |
//| This EA is a byte-for-byte copy of the EntryTimeframe_NoDuplicate  |
//| build with EXACTLY ONE thing changed: the higher-timeframe BIAS    |
//| GATE. Everything else — the LUCC/LDCC reference search, the CC     |
//| confirmation, the RC retest, the entry timeframe, SL/TP, BE @1.2R, |
//| the 50% partial @2R, the 82%->50% target-% profit lock, risk       |
//| sizing, the duplicate-entry protection, and the full CSV logging   |
//| harness — is untouched, so any performance difference is           |
//| attributable SOLELY to the new bias architecture.                  |
//|                                                                    |
//| WHAT CHANGED. The old gate was a single Weekly (1W) pattern check  |
//| (BuyBiasAllowed/SellBiasAllowed). It is replaced by a THREE-TIER,  |
//| CONNECTION-CONSTRAINED multi-timeframe gate. The same seven bias   |
//| patterns (BC, DC, BSB/SBS, 3DP, 3DP-DS, N-3DP, N-DS) are evaluated |
//| on ELEVEN timeframes grouped into three tiers:                     |
//|   Tier 1 (monthly): 1M, 2M, 3M, 4M, 6M                             |
//|   Tier 2 (weekly ): 2W, 10D, 1W, 5D                                |
//|   Tier 3 (daily  ): 2D, 1D                                         |
//| The tiers are NOT fully connected. Only specific timeframe-to-     |
//| timeframe paths are valid, yielding exactly 22 valid bias paths    |
//| (see g_paths[] below). A BUY is permitted only when a COMPLETE     |
//| valid CONNECTED buy path exists — a Tier-1 TF with a buy pattern   |
//| -> a PERMITTED Tier-2 TF with a buy pattern -> a PERMITTED Tier-3  |
//| TF with a buy pattern. SELL is the mirror, evaluated independently |
//| (the "BOTH" philosophy: buy and sell each stand on their own; the  |
//| presence of the opposite side's patterns is irrelevant). There is  |
//| NO Neutral state and NO price-invalidation on the new gate.        |
//|                                                                    |
//| TARGETS ARE 100% IDENTICAL to the existing EA. The single entry    |
//| engine still computes its target off the Weekly (1W) Candle-1      |
//| (with the BC/DC -> Daily-C1 switch) exactly as before; the 3-tier  |
//| gate ONLY decides whether trading is permitted, never the target.  |
//| The Weekly bias state (g_bias) is still computed for that target   |
//| logic; it just no longer gates entries.                            |
//|                                                                    |
//| SYNTHETIC TIMEFRAMES are CALENDAR-ALIGNED: 2M = Jan-Feb/Mar-Apr..; |
//| 3M = calendar quarters; 4M = Jan-Apr/May-Aug/Sep-Dec; 6M = Jan-Jun/|
//| Jul-Dec (all from MN1, anchored to January). 2W = fixed two-week   |
//| blocks from the Monday epoch; 5D/10D/2D = Monday/week-anchored     |
//| trading-day blocks (5D = one Mon-Fri week, 10D = two weeks, 2D =   |
//| Mon-Tue/Wed-Thu/Fri pairs), all from D1. (This differs from the    |
//| legacy Thursday-anchored 5D/10D used by the multi-engine build.)   |
//|                                                                    |
//| LOGGING: the trade CSV gains appended columns describing the       |
//| bias path that permitted the trade — Tier1/2/3 direction,          |
//| qualifying timeframes and qualifying patterns, the chosen bias     |
//| path, the pattern on each tier of that path, the trade direction,  |
//| the entry pattern (LUCC/LDCC) and the entry timeframe.             |
//|                                                                    |
//| The multi-engine machinery is retained but COLLAPSED to a single   |
//| engine (NUM_ENGINES = 1, the 1W->1D->EntryTF engine). The          |
//| duplicate-entry protection code is kept intact (inert with one     |
//| engine, but preserved so the two builds stay structurally aligned).|
//|                                                                    |
//| ==== V11 FIXES ================================================== |
...
//======================================================================
// 3-TIER build — the multi-engine machinery is retained but COLLAPSED to
// a SINGLE engine: 1W -> 1D -> EntryTF. This engine's Weekly (1W) layer
// still drives the TARGET (Weekly Candle-1, with the BC/DC -> Daily-C1
// switch) and its Daily (1D) layer still drives the mandatory Daily
// confirmation — both exactly as in the existing EA, so targets are 100%
// identical. What changed is the BIAS GATE only: it is now the 22-path
// connection-constrained 3-tier system (see the tier subsystem further
// down and g_paths[]). The duplicate-entry protection code is kept intact
// (inert with a single engine) so the two builds stay aligned.
//======================================================================
#define ENG_1W1D    0          // the one and only engine (1W -> 1D)
#define NUM_ENGINES 1

// Bias-timeframe "kind" (how W* accessors resolve) and confirmation-timeframe
// "kind" (how D* accessors resolve) for each engine. Retained so the legacy
// synthetic-cache machinery still compiles; the single engine uses BK_W1/CK_D1
// (native Weekly / Daily), so no legacy synthetic cache is ever built.
#define BK_W1   0
#define BK_2W   1
#define BK_5D   2
#define BK_10D  3
...
int    g_engBias[NUM_ENGINES] = { BK_W1 };
int    g_engConf[NUM_ENGINES] = { CK_D1 };
string g_engName[NUM_ENGINES] = { "3TIER" };
...
struct SOpenTrade
{
   ...
   bool     isPrimary;      // V11 (no-dup): true = this record physically sent the order/limit;
                            // false = a MIRROR — a duplicate signal from another engine that shares
                            // the same physical position/ticket (no order of its own) but still gets
                            // the outcome attributed for its engine's stats/logging.
   bool     isSell;
   long     positionId;
   ...
   // --- 3-TIER bias snapshot (frozen at entry, for the appended CSV columns) ---
   string   tierPath;       // the chosen valid connected path, e.g. "3M -> 10D -> 2D"
   string   tierT1Pat, tierT2Pat, tierT3Pat;   // pattern(s) on the chosen path's TF at each tier
   string   tierT1TFs, tierT2TFs, tierT3TFs;   // ALL qualifying timeframes per tier (this direction)
   string   tierT1Pats, tierT2Pats, tierT3Pats;// ALL qualifying patterns per tier (this direction)
};
...
//======================================================================
// V11 (no-dup) — DUPLICATE-ENTRY DETECTION. Two engines produce the same
// physical trade when they fire the SAME entry-TF setup (identical CC bar)
// in the SAME direction with an effectively identical entry / stop / target.
// Since every engine shares the entry-TF (H1/H2) detection, the reference,
// CC, structural stop and market entry are identical across engines; only
// the higher-TF target (and thus the final entry for a Case-1 limit) can
// differ. So we match on: same direction, same CC bar (same market
// structure & timing) and entry/SL/TP within a small tolerance.
//======================================================================
...
// Index of a PRIMARY pending limit that this proposed entry duplicates, else -1.
int FindDuplicatePending(bool isSell, datetime ccTime, double entry, double sl, double tp, double tol)
{
   for(int i = 0; i < ArraySize(g_pendingEntries); i++)
   {
      if(!g_pendingEntries[i].snap.isPrimary) continue;
      if(SameEntry(isSell, ccTime, entry, sl, tp,
                   g_pendingEntries[i].snap.isSell, g_pendingEntries[i].snap.ccTime,
                   g_pendingEntries[i].snap.requestedEntry, g_pendingEntries[i].snap.slPrice,
                   g_pendingEntries[i].snap.tpPrice, tol))
         return i;
   }
   return -1;
}
...
struct SPendingLog
{
   ...
   bool     isPrimary;      // V11 (no-dup): true = physically executed; false = mirror attribution
   bool     isSell;
   long     posId;
   datetime refTime;
   double   refHigh;
   ...
   // --- 3-TIER bias snapshot (carried from SOpenTrade at close, for the CSV) ---
   string   tierPath;
   string   tierT1Pat, tierT2Pat, tierT3Pat;
   string   tierT1TFs, tierT2TFs, tierT3TFs;
   string   tierT1Pats, tierT2Pats, tierT3Pats;
};
...
long g_cntMirrors    = 0;   // V11: duplicate signals attached as mirror owners (no extra order sent)
long g_rejPosOpen    = 0;
...
void UpdateOpenExcursions(double bid, double ask)
{
   ...
   for(int i = 0; i < ArraySize(g_openTrades); i++)
   {
      if(!g_openTrades[i].isPrimary) continue;   // V11: mirrors inherit the primary's excursion at close
...
void UpdateBreakeven(double bid, double ask)
{
   ...
   for(int i = 0; i < ArraySize(g_openTrades); i++)
   {
      if(!g_openTrades[i].isPrimary) continue;   // V11: only the physical trade is managed
...
//======================================================================
//======================================================================
//  3-TIER, CONNECTION-CONSTRAINED BIAS GATE
//======================================================================
//  Replaces the single-Weekly bias gate. The SAME seven bias patterns are
//  evaluated on ELEVEN timeframes grouped into three tiers, connected by a
//  fixed adjacency (22 valid paths). A direction is permitted only when a
//  COMPLETE valid connected path of that direction exists:
//     Tier-1 TF w/ pattern -> permitted Tier-2 TF w/ pattern
//                          -> permitted Tier-3 TF w/ pattern
//  BUY and SELL are evaluated independently (BOTH philosophy). No Neutral
//  state, no price-invalidation. The gate NEVER touches the target logic
//  (targets remain the Weekly-C1 computation of the single 1W->1D engine).
//======================================================================
#define TF_1M   0
#define TF_2M   1
#define TF_3M   2
...
string TierTFName(int tf)
{
   switch(tf)
   {
      case TF_1M:  return "1M";   case TF_2M:  return "2M";  case TF_3M: return "3M";
      case TF_4M:  return "4M";   case TF_6M:  return "6M";  case TF_2W: return "2W";
      case TF_10D: return "10D";  case TF_1W:  return "1W";  case TF_5D: return "5D";
      case TF_2D:  return "2D";   case TF_1D:  return "1D";
   }
   return "?";
}

// The 22 valid bias paths (Tier1 -> Tier2 -> Tier3). The gate can NEVER
// produce a path outside this list. Adjacency:
//   1M/2M/3M -> {2W,10D,1W,5D};   4M/6M -> {2W,10D} only
//   2W -> 2D;  10D -> 2D;  1W -> {2D,1D};  5D -> {2D,1D}
#define NUM_PATHS 22
int g_pathT1[NUM_PATHS] = { TF_1M,TF_1M,TF_1M,TF_1M,TF_1M,TF_1M,
                            TF_2M,TF_2M,TF_2M,TF_2M,TF_2M,TF_2M,
                            TF_3M,TF_3M,TF_3M,TF_3M,TF_3M,TF_3M,
                            TF_4M,TF_4M, TF_6M,TF_6M };
int g_pathT2[NUM_PATHS] = { TF_2W,TF_10D,TF_1W,TF_1W,TF_5D,TF_5D,
                            TF_2W,TF_10D,TF_1W,TF_1W,TF_5D,TF_5D,
                            TF_2W,TF_10D,TF_1W,TF_1W,TF_5D,TF_5D,
                            TF_2W,TF_10D, TF_2W,TF_10D };
int g_pathT3[NUM_PATHS] = { TF_2D,TF_2D, TF_2D,TF_1D, TF_2D,TF_1D,
                            TF_2D,TF_2D, TF_2D,TF_1D, TF_2D,TF_1D,
                            TF_2D,TF_2D, TF_2D,TF_1D, TF_2D,TF_1D,
                            TF_2D,TF_2D, TF_2D,TF_2D };

// Which tier a timeframe belongs to (1/2/3), for validation only.
int TierOf(int tf)
{
   if(tf==TF_1M||tf==TF_2M||tf==TF_3M||tf==TF_4M||tf==TF_6M) return 1;
   if(tf==TF_2W||tf==TF_10D||tf==TF_1W||tf==TF_5D)           return 2;
   return 3;
}

//----------------------------------------------------------------------
// Synthetic candle caches for the tier system. Native frames (1M=MN1,
// 1W=W1, 1D=D1) are read directly; the other eight are aggregated here.
// All CALENDAR-ALIGNED: monthly frames anchored to January (from MN1),
// 2W = fixed 2-week blocks from the Monday epoch (from W1), 5D/10D/2D =
// Monday/week-anchored trading-day blocks (from D1).
//----------------------------------------------------------------------
double   g_t2mO[],  g_t2mH[],  g_t2mL[],  g_t2mC[];  datetime g_t2mT[];  int g_n2m  = 0;
double   g_t3mO[],  g_t3mH[],  g_t3mL[],  g_t3mC[];  datetime g_t3mT[];  int g_n3m  = 0;
double   g_t4mO[],  g_t4mH[],  g_t4mL[],  g_t4mC[];  datetime g_t4mT[];  int g_n4m  = 0;
double   g_t6mO[],  g_t6mH[],  g_t6mL[],  g_t6mC[];  datetime g_t6mT[];  int g_n6m  = 0;
double   g_t2wO[],  g_t2wH[],  g_t2wL[],  g_t2wC[];  datetime g_t2wT[];  int g_nt2w = 0;
double   g_t5dO[],  g_t5dH[],  g_t5dL[],  g_t5dC[];  datetime g_t5dT[];  int g_nt5d = 0;
double   g_t10dO[], g_t10dH[], g_t10dL[], g_t10dC[]; datetime g_t10dT[]; int g_nt10d= 0;
double   g_t2dO[],  g_t2dH[],  g_t2dL[],  g_t2dC[];  datetime g_t2dT[];  int g_nt2d = 0;
datetime g_b2m=0, g_b3m=0, g_b4m=0, g_b6m=0, g_bt2w=0, g_bt5d=0, g_bt10d=0, g_bt2d=0;

// Calendar-month bucket id: groups of N consecutive months anchored to
// January (12 % N == 0 for N in {2,3,4,6}, so Jan always starts a block).
long BucketMonth(datetime t, int N)
{
   MqlDateTime dt; TimeToStruct(t, dt);
   long monthIndex = (long)dt.year * 12 + (long)(dt.mon - 1);
   long q = monthIndex / N;
   if(monthIndex < 0 && (monthIndex % N) != 0) q--;
   return q;
}
long TierBucket(datetime bt, int mode, int groupN, int anchor)
{
   if(mode == 2) return BucketMonth(bt, groupN);
   if(mode == 1) return BucketWeek(bt, groupN);
   return BucketWeekday(bt, groupN, anchor);
}
// Aggregate native bars of `src` into non-overlapping tier blocks (mode:
// 0=weekday-index, 1=week-index, 2=calendar-month). Same block convention
// as the legacy BuildSynthetic: index 0 = newest/forming block, O = open of
// the oldest bar in the block, C = close of the newest, H/L = extremes.
void BuildTier(ENUM_TIMEFRAMES src, int mode, int groupN, int anchor,
               double &aO[], double &aH[], double &aL[], double &aC[], datetime &aT[], int &n)
{
   int bars = iBars(_Symbol, src);
   int need = MathMin(bars, SYN_MAX * groupN + groupN + 4);
   ArrayResize(aO, SYN_MAX); ArrayResize(aH, SYN_MAX); ArrayResize(aL, SYN_MAX);
   ArrayResize(aC, SYN_MAX); ArrayResize(aT, SYN_MAX);
   n = 0;
   ...
   for(int s = 0; s < need && idx < SYN_MAX - 1; s++)
   {
      datetime bt = iTime(_Symbol, src, s);
      if(bt == 0) break;
      long b = TierBucket(bt, mode, groupN, anchor);
      double o = iOpen(_Symbol, src, s), h = iHigh(_Symbol, src, s),
             l = iLow(_Symbol, src, s),  c = iClose(_Symbol, src, s);
      if(b != curB)
      {
         idx++; curB = b;
         aC[idx] = c; aH[idx] = h; aL[idx] = l; aO[idx] = o; aT[idx] = bt;
      }
      else
      {
         aH[idx] = MathMax(aH[idx], h); aL[idx] = MathMin(aL[idx], l);
...
// Dirty-checked rebuild + live forming-block refresh (cheap to call per tick).
void RebuildTierFrame(ENUM_TIMEFRAMES src, int mode, int groupN, int anchor,
                      double &aO[], double &aH[], double &aL[], double &aC[], datetime &aT[],
                      int &n, datetime &built)
{
   datetime t0 = iTime(_Symbol, src, 0);
   if(t0 != built) { BuildTier(src, mode, groupN, anchor, aO, aH, aL, aC, aT, n); built = t0; }
   if(n > 0) { aH[0] = MathMax(aH[0], iHigh(_Symbol, src, 0));
               aL[0] = MathMin(aL[0], iLow(_Symbol, src, 0));
               aC[0] = iClose(_Symbol, src, 0); }
}
void RebuildTierCaches()
{
   RebuildTierFrame(PERIOD_MN1, 2, 2, 0, g_t2mO,  g_t2mH,  g_t2mL,  g_t2mC,  g_t2mT,  g_n2m,  g_b2m);
   RebuildTierFrame(PERIOD_MN1, 2, 3, 0, g_t3mO,  g_t3mH,  g_t3mL,  g_t3mC,  g_t3mT,  g_n3m,  g_b3m);
   RebuildTierFrame(PERIOD_MN1, 2, 4, 0, g_t4mO,  g_t4mH,  g_t4mL,  g_t4mC,  g_t4mT,  g_n4m,  g_b4m);
   RebuildTierFrame(PERIOD_MN1, 2, 6, 0, g_t6mO,  g_t6mH,  g_t6mL,  g_t6mC,  g_t6mT,  g_n6m,  g_b6m);
   RebuildTierFrame(PERIOD_W1,  1, 2, 0, g_t2wO,  g_t2wH,  g_t2wL,  g_t2wC,  g_t2wT,  g_nt2w, g_bt2w);
   RebuildTierFrame(PERIOD_D1,  0, 5, 0, g_t5dO,  g_t5dH,  g_t5dL,  g_t5dC,  g_t5dT,  g_nt5d, g_bt5d);
   RebuildTierFrame(PERIOD_D1,  0,10, 0, g_t10dO, g_t10dH, g_t10dL, g_t10dC, g_t10dT, g_nt10d,g_bt10d);
   RebuildTierFrame(PERIOD_D1,  0, 2, 0, g_t2dO,  g_t2dH,  g_t2dL,  g_t2dC,  g_t2dT,  g_nt2d, g_bt2d);
}

// Generic OHLC accessors keyed by timeframe id (offset 0 = forming block).
double TF_O(int tf, int s)
{
   switch(tf)
   {
      case TF_1M:  return iOpen(_Symbol, PERIOD_MN1, s);
      case TF_2M:  return (s>=0&&s<g_n2m)  ? g_t2mO[s]  : 0.0;
      case TF_3M:  return (s>=0&&s<g_n3m)  ? g_t3mO[s]  : 0.0;
      case TF_4M:  return (s>=0&&s<g_n4m)  ? g_t4mO[s]  : 0.0;
      case TF_6M:  return (s>=0&&s<g_n6m)  ? g_t6mO[s]  : 0.0;
      case TF_2W:  return (s>=0&&s<g_nt2w) ? g_t2wO[s]  : 0.0;
      case TF_10D: return (s>=0&&s<g_nt10d)? g_t10dO[s] : 0.0;
      case TF_1W:  return iOpen(_Symbol, PERIOD_W1, s);
      case TF_5D:  return (s>=0&&s<g_nt5d) ? g_t5dO[s]  : 0.0;
      case TF_2D:  return (s>=0&&s<g_nt2d) ? g_t2dO[s]  : 0.0;
      case TF_1D:  return iOpen(_Symbol, PERIOD_D1, s);
   }
   return 0.0;
}
double TF_H(int tf, int s)
{
   switch(tf)
   {
      case TF_1M:  return iHigh(_Symbol, PERIOD_MN1, s);
      case TF_2M:  return (s>=0&&s<g_n2m)  ? g_t2mH[s]  : 0.0;
      case TF_3M:  return (s>=0&&s<g_n3m)  ? g_t3mH[s]  : 0.0;
      case TF_4M:  return (s>=0&&s<g_n4m)  ? g_t4mH[s]  : 0.0;
      case TF_6M:  return (s>=0&&s<g_n6m)  ? g_t6mH[s]  : 0.0;
      case TF_2W:  return (s>=0&&s<g_nt2w) ? g_t2wH[s]  : 0.0;
      case TF_10D: return (s>=0&&s<g_nt10d)? g_t10dH[s] : 0.0;
      case TF_1W:  return iHigh(_Symbol, PERIOD_W1, s);
      case TF_5D:  return (s>=0&&s<g_nt5d) ? g_t5dH[s]  : 0.0;
      case TF_2D:  return (s>=0&&s<g_nt2d) ? g_t2dH[s]  : 0.0;
      case TF_1D:  return iHigh(_Symbol, PERIOD_D1, s);
   }
   return 0.0;
}
double TF_L(int tf, int s)
{
   switch(tf)
   {
      case TF_1M:  return iLow(_Symbol, PERIOD_MN1, s);
      case TF_2M:  return (s>=0&&s<g_n2m)  ? g_t2mL[s]  : 0.0;
      case TF_3M:  return (s>=0&&s<g_n3m)  ? g_t3mL[s]  : 0.0;
      case TF_4M:  return (s>=0&&s<g_n4m)  ? g_t4mL[s]  : 0.0;
      case TF_6M:  return (s>=0&&s<g_n6m)  ? g_t6mL[s]  : 0.0;
      case TF_2W:  return (s>=0&&s<g_nt2w) ? g_t2wL[s]  : 0.0;
      case TF_10D: return (s>=0&&s<g_nt10d)? g_t10dL[s] : 0.0;
      case TF_1W:  return iLow(_Symbol, PERIOD_W1, s);
      case TF_5D:  return (s>=0&&s<g_nt5d) ? g_t5dL[s]  : 0.0;
      case TF_2D:  return (s>=0&&s<g_nt2d) ? g_t2dL[s]  : 0.0;
      case TF_1D:  return iLow(_Symbol, PERIOD_D1, s);
   }
   return 0.0;
}
double TF_C(int tf, int s)
{
   switch(tf)
   {
      case TF_1M:  return iClose(_Symbol, PERIOD_MN1, s);
      case TF_2M:  return (s>=0&&s<g_n2m)  ? g_t2mC[s]  : 0.0;
      case TF_3M:  return (s>=0&&s<g_n3m)  ? g_t3mC[s]  : 0.0;
      case TF_4M:  return (s>=0&&s<g_n4m)  ? g_t4mC[s]  : 0.0;
      case TF_6M:  return (s>=0&&s<g_n6m)  ? g_t6mC[s]  : 0.0;
      case TF_2W:  return (s>=0&&s<g_nt2w) ? g_t2wC[s]  : 0.0;
      case TF_10D: return (s>=0&&s<g_nt10d)? g_t10dC[s] : 0.0;
      case TF_1W:  return iClose(_Symbol, PERIOD_W1, s);
      case TF_5D:  return (s>=0&&s<g_nt5d) ? g_t5dC[s]  : 0.0;
      case TF_2D:  return (s>=0&&s<g_nt2d) ? g_t2dC[s]  : 0.0;
      case TF_1D:  return iClose(_Symbol, PERIOD_D1, s);
   }
   return 0.0;
}
int TF_Bars(int tf)
{
   switch(tf)
   {
      case TF_1M:  return iBars(_Symbol, PERIOD_MN1);
      case TF_2M:  return g_n2m;
      case TF_3M:  return g_n3m;
      case TF_4M:  return g_n4m;
      case TF_6M:  return g_n6m;
      case TF_2W:  return g_nt2w;
      case TF_10D: return g_nt10d;
      case TF_1W:  return iBars(_Symbol, PERIOD_W1);
      case TF_5D:  return g_nt5d;
      case TF_2D:  return g_nt2d;
      case TF_1D:  return iBars(_Symbol, PERIOD_D1);
   }
   return 0;
}

// The SAME seven patterns as the Weekly/Daily bias set, formulas identical,
// evaluated on any timeframe id via the generic accessors above.
bool Tier_BC_Buy(int t)   { return TF_C(t,1) > TF_H(t,2); }
bool Tier_DC_Buy(int t)   { return TF_L(t,1) < TF_L(t,2) && TF_H(t,1) > TF_H(t,2) && TF_C(t,1) > TF_O(t,1); }
bool Tier_BSB_Buy(int t)  { return TF_C(t,2) > TF_O(t,2) && TF_H(t,1) <= TF_H(t,2) && TF_L(t,1) >= TF_L(t,2); }
bool Tier_3DP_Buy(int t)  { return TF_C(t,1) >= TF_L(t,2) && TF_C(t,1) <= TF_H(t,2) && TF_L(t,0) < TF_L(t,1) && TF_L(t,0) < TF_L(t,2); }
bool Tier_DS_Buy(int t)   { return TF_C(t,1) < TF_L(t,2) && TF_C(t,1) < TF_L(t,3) &&
                                   TF_C(t,2) >= TF_L(t,1) && TF_C(t,2) >= TF_L(t,2) && TF_C(t,2) >= TF_L(t,3) &&
                                   TF_L(t,0) < TF_L(t,1); }
bool Tier_N3DP_Buy(int t) { return TF_H(t,1) <= TF_H(t,2) && TF_L(t,1) >= TF_L(t,2) && TF_L(t,0) < TF_L(t,1) && TF_L(t,0) < TF_L(t,2); }
bool Tier_NDS_Buy(int t)  { return TF_H(t,2) <= TF_H(t,3) && TF_L(t,2) >= TF_L(t,3) &&
                                   TF_C(t,1) < TF_L(t,2) && TF_C(t,1) < TF_L(t,3) &&
                                   TF_L(t,0) < TF_L(t,1) && TF_L(t,0) < TF_L(t,2) && TF_L(t,0) < TF_L(t,3) &&
                                   TF_H(t,1) <= TF_H(t,3) && TF_H(t,0) <= TF_H(t,3); }

bool Tier_BC_Sell(int t)   { return TF_C(t,1) < TF_L(t,2); }
bool Tier_DC_Sell(int t)   { return TF_H(t,1) > TF_H(t,2) && TF_L(t,1) < TF_L(t,2) && TF_C(t,1) < TF_O(t,1); }
bool Tier_SBS_Sell(int t)  { return TF_C(t,2) < TF_O(t,2) && TF_H(t,1) <= TF_H(t,2) && TF_L(t,1) >= TF_L(t,2); }
bool Tier_3DP_Sell(int t)  { return TF_C(t,1) >= TF_L(t,2) && TF_C(t,1) <= TF_H(t,2) && TF_H(t,0) > TF_H(t,1) && TF_H(t,0) > TF_H(t,2); }
bool Tier_DS_Sell(int t)   { return TF_C(t,1) > TF_H(t,2) && TF_C(t,1) > TF_H(t,3) &&
                                    TF_C(t,2) <= TF_H(t,1) && TF_C(t,2) <= TF_H(t,2) && TF_C(t,2) <= TF_H(t,3) &&
                                    TF_H(t,0) > TF_H(t,1); }
bool Tier_N3DP_Sell(int t) { return TF_H(t,1) <= TF_H(t,2) && TF_L(t,1) >= TF_L(t,2) && TF_H(t,0) > TF_H(t,1) && TF_H(t,0) > TF_H(t,2); }
bool Tier_NDS_Sell(int t)  { return TF_H(t,2) <= TF_H(t,3) && TF_L(t,2) >= TF_L(t,3) &&
                                    TF_C(t,1) > TF_H(t,2) && TF_C(t,1) > TF_H(t,3) &&
                                    TF_H(t,0) > TF_H(t,1) && TF_H(t,0) > TF_H(t,2) && TF_H(t,0) > TF_H(t,3) &&
                                    TF_L(t,1) >= TF_L(t,3) && TF_L(t,0) >= TF_L(t,3); }

string TierBuyText(int t)
{
   if(TF_Bars(t) < 4) return "-";
   string s = "";
   if(Tier_BC_Buy(t))   s += "BC ";
   if(Tier_DC_Buy(t))   s += "DC ";
   if(Tier_BSB_Buy(t))  s += "BSB ";
   if(Tier_3DP_Buy(t))  s += "3DP ";
   if(Tier_DS_Buy(t))   s += "3DP-DS ";
   if(Tier_N3DP_Buy(t)) s += "N-3DP ";
   if(Tier_NDS_Buy(t))  s += "N-DS ";
   return (s == "") ? "-" : s;
}
string TierSellText(int t)
{
   if(TF_Bars(t) < 4) return "-";
   string s = "";
   if(Tier_BC_Sell(t))   s += "BC ";
   if(Tier_DC_Sell(t))   s += "DC ";
   if(Tier_SBS_Sell(t))  s += "SBS ";
   if(Tier_3DP_Sell(t))  s += "3DP ";
   if(Tier_DS_Sell(t))   s += "3DP-DS ";
   if(Tier_N3DP_Sell(t)) s += "N-3DP ";
   if(Tier_NDS_Sell(t))  s += "N-DS ";
   return (s == "") ? "-" : s;
}

// Append `token` to a space-separated accumulator only if not already present.
void TierAppendUnique(string &acc, string token)
{
   if(token == "" ) return;
   string parts[];
   int n = StringSplit(acc, ' ', parts);
   for(int i = 0; i < n; i++)
      if(parts[i] == token) return;
   acc = (acc == "") ? token : acc + " " + token;
}
// Append every token of a pattern text ("BC DC 3DP ") into the accumulator (dedup).
void TierAppendTokens(string &acc, string tokens)
{
   if(tokens == "-" || tokens == "") return;
   string parts[];
   int n = StringSplit(tokens, ' ', parts);
   for(int i = 0; i < n; i++)
      if(StringLen(parts[i]) > 0) TierAppendUnique(acc, parts[i]);
}

//----------------------------------------------------------------------
// The 3-tier gate state (recomputed each tick from closed candles only).
//----------------------------------------------------------------------
struct STierBias
{
   bool   buyAllowed, sellAllowed;
   // BUY: ALL qualifying timeframes / patterns per tier (union over participating full paths)
   string buyT1TFs, buyT2TFs, buyT3TFs;
   string buyT1Pats, buyT2Pats, buyT3Pats;
   string buyPath;                        // first valid buy path "3M -> 10D -> 2D"
   string buyT1Pat, buyT2Pat, buyT3Pat;   // pattern text on that chosen path's TF per tier
   // SELL (mirror)
   string sellT1TFs, sellT2TFs, sellT3TFs;
   string sellT1Pats, sellT2Pats, sellT3Pats;
   string sellPath;
   string sellT1Pat, sellT2Pat, sellT3Pat;
};
STierBias g_tier;

// Recompute the whole 3-tier gate. Closed-bar only (no price invalidation),
// so it needs no bid/ask. Call once per tick BEFORE the entry path.
void ComputeTierBias()
{
   RebuildTierCaches();

   // Per-TF pattern presence for both directions.
   string btxt[NUM_TF], stxt[NUM_TF];
   bool   bpres[NUM_TF], spres[NUM_TF];
   for(int i = 0; i < NUM_TF; i++)
   {
      btxt[i] = TierBuyText(i);  bpres[i] = (btxt[i] != "-");
      stxt[i] = TierSellText(i); spres[i] = (stxt[i] != "-");
   }

   // Reset state.
   g_tier.buyAllowed = false; g_tier.sellAllowed = false;
   g_tier.buyT1TFs=""; g_tier.buyT2TFs=""; g_tier.buyT3TFs="";
   g_tier.buyT1Pats=""; g_tier.buyT2Pats=""; g_tier.buyT3Pats="";
   g_tier.buyPath="-"; g_tier.buyT1Pat="-"; g_tier.buyT2Pat="-"; g_tier.buyT3Pat="-";
   g_tier.sellT1TFs=""; g_tier.sellT2TFs=""; g_tier.sellT3TFs="";
   g_tier.sellT1Pats=""; g_tier.sellT2Pats=""; g_tier.sellT3Pats="";
   g_tier.sellPath="-"; g_tier.sellT1Pat="-"; g_tier.sellT2Pat="-"; g_tier.sellT3Pat="-";

   // Walk the 22 fixed paths for each direction independently.
   for(int p = 0; p < NUM_PATHS; p++)
   {
      int t1 = g_pathT1[p], t2 = g_pathT2[p], t3 = g_pathT3[p];

      if(bpres[t1] && bpres[t2] && bpres[t3])
      {
         if(!g_tier.buyAllowed)   // first qualifying buy path is the "chosen" one
         {
            g_tier.buyPath  = TierTFName(t1) + " -> " + TierTFName(t2) + " -> " + TierTFName(t3);
            g_tier.buyT1Pat = btxt[t1]; g_tier.buyT2Pat = btxt[t2]; g_tier.buyT3Pat = btxt[t3];
         }
         g_tier.buyAllowed = true;
         TierAppendUnique(g_tier.buyT1TFs, TierTFName(t1));
         TierAppendUnique(g_tier.buyT2TFs, TierTFName(t2));
         TierAppendUnique(g_tier.buyT3TFs, TierTFName(t3));
         TierAppendTokens(g_tier.buyT1Pats, btxt[t1]);
         TierAppendTokens(g_tier.buyT2Pats, btxt[t2]);
         TierAppendTokens(g_tier.buyT3Pats, btxt[t3]);
      }
      if(spres[t1] && spres[t2] && spres[t3])
      {
         if(!g_tier.sellAllowed)
         {
            g_tier.sellPath  = TierTFName(t1) + " -> " + TierTFName(t2) + " -> " + TierTFName(t3);
            g_tier.sellT1Pat = stxt[t1]; g_tier.sellT2Pat = stxt[t2]; g_tier.sellT3Pat = stxt[t3];
         }
         g_tier.sellAllowed = true;
         TierAppendUnique(g_tier.sellT1TFs, TierTFName(t1));
         TierAppendUnique(g_tier.sellT2TFs, TierTFName(t2));
         TierAppendUnique(g_tier.sellT3TFs, TierTFName(t3));
         TierAppendTokens(g_tier.sellT1Pats, stxt[t1]);
         TierAppendTokens(g_tier.sellT2Pats, stxt[t2]);
         TierAppendTokens(g_tier.sellT3Pats, stxt[t3]);
      }
   }

   if(g_tier.buyT1TFs=="")  g_tier.buyT1TFs="-";
   if(g_tier.buyT2TFs=="")  g_tier.buyT2TFs="-";
   if(g_tier.buyT3TFs=="")  g_tier.buyT3TFs="-";
   if(g_tier.buyT1Pats=="") g_tier.buyT1Pats="-";
   if(g_tier.buyT2Pats=="") g_tier.buyT2Pats="-";
   if(g_tier.buyT3Pats=="") g_tier.buyT3Pats="-";
   if(g_tier.sellT1TFs=="") g_tier.sellT1TFs="-";
   if(g_tier.sellT2TFs=="") g_tier.sellT2TFs="-";
   if(g_tier.sellT3TFs=="") g_tier.sellT3TFs="-";
   if(g_tier.sellT1Pats=="")g_tier.sellT1Pats="-";
   if(g_tier.sellT2Pats=="")g_tier.sellT2Pats="-";
   if(g_tier.sellT3Pats=="")g_tier.sellT3Pats="-";
}

// The 3-TIER GATE. A direction is tradeable only when a complete valid
// connected path of that direction exists. With the bias filter off the gate
// is transparent (always allowed), exactly like the legacy switch.
bool BuyBiasAllowed()  { return !InpUseBiasFilter || g_tier.buyAllowed; }
bool SellBiasAllowed() { return !InpUseBiasFilter || g_tier.sellAllowed; }
...
bool TryOpen(bool isSell, SSignalState &st)
{
   ...
   // 3-TIER bias gate — a Retest Candle only becomes an actionable entry if a
   // complete valid CONNECTED path of this direction exists (see ComputeTierBias).
   // Buy and sell are gated independently (BOTH philosophy), so each side can
   // trade on its own. This gates ONLY the entry; the LUCC/LDCC/CC/RC detection
   // above is untouched, and the target logic below is unaffected.
   bool biasOk = isSell ? SellBiasAllowed() : BuyBiasAllowed();
   if(!biasOk)
   {
      g_rejBias++;
      LogSkip(isSell, st, "SkipBias");
      Dbg(StringFormat("%s retest REJECT: 3-tier gate (no valid connected %s path) | buyPath=[%s] sellPath=[%s]",
                       side, side, g_tier.buyPath, g_tier.sellPath));
      return false;
   }
   ...
   ot.isPrimary       = true;             // V11: default; flipped to false if this is a mirror of another engine
   ot.isSell          = isSell;
   ot.positionId      = 0;
   ...
   // --- 3-TIER bias snapshot on THIS trade's side (for the appended CSV columns) ---
   if(isSell)
   {
      ot.tierPath  = g_tier.sellPath;
      ot.tierT1Pat = g_tier.sellT1Pat;  ot.tierT2Pat = g_tier.sellT2Pat;  ot.tierT3Pat = g_tier.sellT3Pat;
      ot.tierT1TFs = g_tier.sellT1TFs;  ot.tierT2TFs = g_tier.sellT2TFs;  ot.tierT3TFs = g_tier.sellT3TFs;
      ot.tierT1Pats= g_tier.sellT1Pats; ot.tierT2Pats= g_tier.sellT2Pats; ot.tierT3Pats= g_tier.sellT3Pats;
   }
   else
   {
      ot.tierPath  = g_tier.buyPath;
      ot.tierT1Pat = g_tier.buyT1Pat;   ot.tierT2Pat = g_tier.buyT2Pat;   ot.tierT3Pat = g_tier.buyT3Pat;
      ot.tierT1TFs = g_tier.buyT1TFs;   ot.tierT2TFs = g_tier.buyT2TFs;   ot.tierT3TFs = g_tier.buyT3TFs;
      ot.tierT1Pats= g_tier.buyT1Pats;  ot.tierT2Pats= g_tier.buyT2Pats;  ot.tierT3Pats= g_tier.buyT3Pats;
   }

   // === V11 (no-dup): if another engine already produced this exact entry
   //     (same CC bar, direction, entry/SL/TP within tolerance), attach this
   //     engine as a MIRROR owner of that ONE physical trade instead of sending
   //     a second order. The outcome is attributed to this engine at close. ===
   double dupTol = MathMax(3.0 * _Point, dist * 0.03);
   int dOpen = FindDuplicateOpen(isSell, ot.ccTime, entry, sl, tp, dupTol);
   if(dOpen >= 0)
   {
      SOpenTrade prim = g_openTrades[dOpen];
      ot.isPrimary    = false;
      ot.positionId   = prim.positionId;        // share the physical position
      ot.entryType    = prim.entryType;
      ot.entryTime    = prim.entryTime;
      ot.entryPrice   = prim.entryPrice;
      ot.slPrice      = prim.slPrice;   ot.tpPrice = prim.tpPrice;
      ot.lots         = prim.lots;      ot.initRiskDist = prim.initRiskDist;
      ot.beLevelPrice = prim.entryPrice;
      ot.mfePrice     = prim.entryPrice; ot.maePrice = prim.entryPrice;
      ot.limitPlacedTime = 0;
      int oi = ArraySize(g_openTrades); ArrayResize(g_openTrades, oi + 1); g_openTrades[oi] = ot;
      st.tradesToday++; g_cntMirrors++;
      Dbg(StringFormat("%s MIRROR of posId=%d (engine %s): duplicate signal, no new order (openTrades=%d)",
                       side, prim.positionId, EngineName(g_curEng), ArraySize(g_openTrades)));
      return true;
   }
   int dPend = FindDuplicatePending(isSell, ot.ccTime, entry, sl, tp, dupTol);
   if(dPend >= 0)
   {
      // Mirror a not-yet-filled Case-1 limit: share its order ticket, so both
      // are promoted together when the one physical limit fills (HandleFill),
      // and cancelled together if the limit is cancelled.
      ot.isPrimary = false;
      int pe = ArraySize(g_pendingEntries); ArrayResize(g_pendingEntries, pe + 1);
      g_pendingEntries[pe].snap        = ot;
      g_pendingEntries[pe].orderTicket = g_pendingEntries[dPend].orderTicket;  // shared ticket
      g_pendingEntries[pe].placedTime  = g_pendingEntries[dPend].placedTime;
      g_pendingEntries[pe].awayExtreme = entry;
      st.tradesToday++; g_cntMirrors++;
      Dbg(StringFormat("%s MIRROR PENDING of ticket=%d (engine %s): duplicate limit, no new order",
                       side, (long)g_pendingEntries[dPend].orderTicket, EngineName(g_curEng)));
      return true;
   }
...
void HandleFill(ulong dealTicket)
{
   ...
   datetime fillTime    = (datetime)HistoryDealGetInteger(dealTicket, DEAL_TIME);
   double   fill        = HistoryDealGetDouble(dealTicket, DEAL_PRICE);

   // V11: promote the primary AND every MIRROR pending sharing this order ticket
   // (they were all the same physical limit) into open trades on the filled
   // position. Each keeps its own engine context; only one order ever existed.
   bool any = false;
   for(int i = ArraySize(g_pendingEntries) - 1; i >= 0; i--)
   {
      if(g_pendingEntries[i].orderTicket != orderTicket) continue;
      SOpenTrade ot = g_pendingEntries[i].snap;
      ot.positionId   = posId;
      ot.entryTime    = fillTime;
      ot.entryPrice   = (fill > 0) ? fill : ot.requestedEntry;
      ot.initRiskDist = MathAbs(ot.entryPrice - ot.slPrice);
      ot.beLevelPrice = ot.entryPrice;
      ot.mfePrice = ot.entryPrice; ot.maePrice = ot.entryPrice;
      int oi = ArraySize(g_openTrades); ArrayResize(g_openTrades, oi + 1); g_openTrades[oi] = ot;
      if(ot.isPrimary) { g_cntLimitsFilled++; g_cntEntries++; }
      ArrayRemove(g_pendingEntries, i, 1);
      any = true;
      Dbg(StringFormat("%s CASE-1 LIMIT FILLED%s: fill=%.5f posId=%d engine=%s",
                       ot.isSell ? "SELL" : "BUY", ot.isPrimary ? "" : " [MIRROR]",
                       ot.entryPrice, posId, EngineName(ot.engine)));
   }
   if(!any)
...
void WritePathBars()
{
   ...
   for(int i = 0; i < ArraySize(g_openTrades); i++)
   {
      if(!g_openTrades[i].isPrimary) continue;   // V11: one path per physical trade (mirrors share it)
...
void WriteTradeLogHeader()
{
   string h[] =
   {
      ...
      "Engine",
      // --- V11 (no-dup): PhysicalExecution = 1 for the ONE real order, 0 for a
      //     mirror attribution row. Rows that share a TradeID are the same
      //     physical trade; sum money only over PhysicalExecution=1 rows. ---
      "PhysicalExecution",
      // --- 3-TIER bias filter: the connected path that permitted this trade ---
      "TradeDirection", "EntryPattern_3T", "EntryTimeframe_3T", "BiasPath",
      "Tier1_Direction", "Tier1_QualifyingTFs", "Tier1_QualifyingPatterns", "Tier1_PathPattern",
      "Tier2_Direction", "Tier2_QualifyingTFs", "Tier2_QualifyingPatterns", "Tier2_PathPattern",
      "Tier3_Direction", "Tier3_QualifyingTFs", "Tier3_QualifyingPatterns", "Tier3_PathPattern"
   };
                      ...
                      bool firstWinOfBias, string outcome)
{
   ...
   g_pending[n].isPrimary          = t.isPrimary;
   ...
   // --- 3-TIER bias snapshot ---
   g_pending[n].tierPath   = t.tierPath;
   g_pending[n].tierT1Pat  = t.tierT1Pat;  g_pending[n].tierT2Pat  = t.tierT2Pat;  g_pending[n].tierT3Pat  = t.tierT3Pat;
   g_pending[n].tierT1TFs  = t.tierT1TFs;  g_pending[n].tierT2TFs  = t.tierT2TFs;  g_pending[n].tierT3TFs  = t.tierT3TFs;
   g_pending[n].tierT1Pats = t.tierT1Pats; g_pending[n].tierT2Pats = t.tierT2Pats; g_pending[n].tierT3Pats = t.tierT3Pats;
...
void FinalizePendingLog(const SPendingLog &p, bool windowComplete)
{
   ...
   string v[] =
   {
      ...
      // --- V7: engine identity ---
      EngineName(p.engine),
      // --- V11 (no-dup): physical vs mirror attribution ---
      p.isPrimary ? "1" : "0",
      // --- 3-TIER bias filter: the connected path that permitted this trade ---
      (p.isSell ? "SELL" : "BUY"), entryPattern, StringSubstr(EnumToString(ENTRY_TF), 7), CsvEscape(p.tierPath),
      (p.isSell ? "SELL" : "BUY"), CsvEscape(p.tierT1TFs), CsvEscape(p.tierT1Pats), CsvEscape(p.tierT1Pat),
      (p.isSell ? "SELL" : "BUY"), CsvEscape(p.tierT2TFs), CsvEscape(p.tierT2Pats), CsvEscape(p.tierT2Pat),
      (p.isSell ? "SELL" : "BUY"), CsvEscape(p.tierT3TFs), CsvEscape(p.tierT3Pats), CsvEscape(p.tierT3Pat)
   };
...
void HandleClose(long closedPosId, datetime exitTime, double exitPrice, string exitReason,
                 double grossProfit, double commission, double swap)
{
   // V11: find the PRIMARY record for this physical position (the one that
   // actually sent the order). Mirrors from other engines share its positionId.
   int pidx = -1;
   for(int i = 0; i < ArraySize(g_openTrades); i++)
      if(g_openTrades[i].positionId == closedPosId && g_openTrades[i].isPrimary) { pidx = i; break; }
   if(pidx < 0)
   ...
   // it was the partial: bank its realized P&L into the PRIMARY record and return.
   if(PositionSelectByTicket((ulong)closedPosId))
   {
      g_openTrades[pidx].partialProfit += grossProfit;
      g_openTrades[pidx].partialComm   += commission;
      g_openTrades[pidx].partialSwap   += swap;
      if(g_openTrades[pidx].partialTime == 0) g_openTrades[pidx].partialTime = exitTime;
      Dbg(StringFormat("%s PARTIAL booked posId=%d partGross=%.2f (position still open, %.2f lots remain)",
                       g_openTrades[pidx].isSell ? "SELL" : "BUY", closedPosId, grossProfit,
                       PositionGetDouble(POSITION_VOLUME)));
      return;
   }

   // FULL close: build the shared realized result from the PRIMARY (fold its
   // partial P&L) and the price-based outcome. This ONE physical result is then
   // attributed to every owner engine (primary + mirrors) below.
   SOpenTrade prim = g_openTrades[pidx];
   double gTot    = grossProfit + prim.partialProfit;
   double cTot    = commission  + prim.partialComm;
   double sTot    = swap        + prim.partialSwap;
   double netProfit = gTot + cTot + sTot;
   string outcome = ClassifyOutcome(prim, exitPrice);       // TP / LOCK / BE / LOSS
   bool   winStop = (outcome == "TP") || (outcome == "LOCK" && InpTargetLockCountsAsWin);

   // Attribute the outcome to EVERY owner of this physical position. Each owner
   // keeps its own engine context (bias/daily patterns, episode, target source)
   // but inherits the primary's trade mechanics and the shared realized P&L, so
   // its stats/logs record the trade even though only one order was executed.
   for(int i = ArraySize(g_openTrades) - 1; i >= 0; i--)
   {
      if(g_openTrades[i].positionId != closedPosId) continue;
      SOpenTrade t = g_openTrades[i];

      if(!t.isPrimary)   // mirrors never ran management — copy the physical mechanics for logging
      {
         t.entryPrice = prim.entryPrice; t.slPrice = prim.slPrice; t.tpPrice = prim.tpPrice;
         t.lots = prim.lots; t.initRiskDist = prim.initRiskDist;
         t.beArmed = prim.beArmed; t.beLevelPrice = prim.beLevelPrice; t.beArmTime = prim.beArmTime;
         t.tgtLockArmed = prim.tgtLockArmed; t.tgtLockPrice = prim.tgtLockPrice;
         t.partialDone = prim.partialDone; t.partialTime = prim.partialTime;
         t.partialPrice = prim.partialPrice; t.partialLots = prim.partialLots;
         t.mfePrice = prim.mfePrice; t.maePrice = prim.maePrice;
         t.maxDDMoney = prim.maxDDMoney; t.secsInProfit = prim.secsInProfit; t.secsInDraw = prim.secsInDraw;
         t.firstFavRTime = prim.firstFavRTime; t.tpHitTime = prim.tpHitTime;
      }
      ...
      Dbg(StringFormat("%s CLOSE%s posId=%d net=%.2f outcome=%s engine=%s%s (owners left=%d)",
                       t.isSell ? "SELL" : "BUY", t.isPrimary ? "" : " [MIRROR]", closedPosId, netProfit,
                       outcome, EngineName(t.engine), firstWin ? " [FIRST-WIN]" : "", ArraySize(g_openTrades) - 1));

      if(InpEnableTradeLog)
         QueuePendingLog(t, exitTime, exitPrice, exitReason, gTot, cTot, sTot, firstWin, outcome);

      ArrayRemove(g_openTrades, i, 1);
   }
}
...
int OnInit()
{
   ...
   // 3-TIER gate — start closed (no direction permitted) until the first tick
   // computes it from closed candles; also seed the log-text fields to "-".
   g_tier.buyAllowed = false; g_tier.sellAllowed = false;
   g_tier.buyPath="-";  g_tier.buyT1Pat="-";  g_tier.buyT2Pat="-";  g_tier.buyT3Pat="-";
   g_tier.buyT1TFs="-"; g_tier.buyT2TFs="-"; g_tier.buyT3TFs="-";
   g_tier.buyT1Pats="-";g_tier.buyT2Pats="-";g_tier.buyT3Pats="-";
   ...
   Dbg(StringFormat("INIT filters: SessionGate=NONE(24/5)  BiasFilter(3-TIER)=%s  BarCloseRetest=%s  Lookback=%d | H1bars=%d W1bars=%d MN1bars=%d riskBal=%.2f",
                    InpUseBiasFilter ? "ON" : "OFF", InpUseBarCloseRetest ? "ON" : "OFF", LOOKBACK,
                    iBars(_Symbol, PERIOD_H1), iBars(_Symbol, PERIOD_W1), iBars(_Symbol, PERIOD_MN1), g_riskBalance));
   Dbg(StringFormat("INIT 3-TIER gate: %d valid connected paths | T1={1M,2M,3M,4M,6M} T2={2W,10D,1W,5D} T3={2D,1D} | target=WeeklyC1 (unchanged), gate is entry-permission only, BOTH philosophy, no invalidation",
                    NUM_PATHS));
...
void OnDeinit(const int reason)
{
   ...
   PrintFormat("LUCC/LDCC EA FUNNEL: refs=%d CCs=%d retestTouches=%d ENTRIES=%d mirrors(dedup)=%d skippedAfterWin=%d pendingExpired=%d dailyBEmoves=%d (StopAfterWinRule=%s, ExpirePendingDaily=%s, DailyConfirm=%s, DailyBE=%s)",
               g_cntRef, g_cntCc, g_cntTouch, g_cntEntries, g_cntMirrors, g_skippedAfterWin, g_expiredPending, g_beDailyMoved,
...
void OnTick()
{
   ...
   // 3-TIER bias gate — recompute from closed candles BEFORE the bar-close
   // entry path (ProcessNewBar -> UpdateDirection -> TryOpen) reads it, so the
   // gate is always fresh when a Retest Candle confirms on the new bar.
   ComputeTierBias();
